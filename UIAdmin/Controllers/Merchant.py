...
class MerchantEditHandler(AdminRequestHandler):
    ...
    def post(self, *args, **kwargs):
        ...
        form = MerchantForm()
        try:
            is_valid = form.valid(self)       #拿到 is_valid，_value_dict，_error_dict
            if is_valid:
                if form._value_dict['county_id'] == '0':
                    form._error_dict['county_id'] == '请选着县ID'
                else:
                ...
                form.init_value(form._value_dict)   #init_value,提交出错时初始化保留数据填充
        except IntegrityError as e:
            error_summary = '商户名称或登录用户必须唯一'
        except Exception as e:
            error_summary = str(e)

        self.render('Merchant/MerchantEdit.html', form=form, crumbs='添加商户', method='POST', summary=error_summary,
                    nid=None)
    ...
    def put(self, *args, **kwargs):
        ...
        merchant_id = self.get_argument('nid',None)
        try:
            is_value = form.valid(self)
            if is_value:
                if form._value_dict['county_id'] == '0':
                    form._error_dict['county_id'] == '请选着区ID'
                else:
        ...
        service = MerchantService(MerchantRepository())
        detail = service.get_merchant_detail_by_nid(merchant_id)
        county_caption = detail.get('county_caption')
        county_id = detail.get('county_id')
        form.county_id.widget.choices.append({'value':county_id,'text':county_caption})
